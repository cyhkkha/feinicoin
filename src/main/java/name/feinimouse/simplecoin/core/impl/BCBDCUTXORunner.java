...
import name.feinimouse.simplecoin.manager.custome.SimpleBCBDCUTXOCenter;
import name.feinimouse.simplecoin.manager.custome.SimpleBCBDCUTXOOrder;
...
public class BCBDCUTXORunner extends SimplecoinRunner {
    ...
    public StatisticsObj run() {
        ...
        Arrays.asList(TEST_COUNT).forEach(size -> {
            ...
            var order = new SimpleBCBDCUTXOOrder(userManager, list);
            var center = new SimpleBCBDCUTXOCenter(order);
            center.activate();
